...
func TestPathComp(t *testing.T) {
	rt2 := &Table2[int]{}
	rt2.Insert(mpp("10.0.0.0/8"), 1)
	rt2.Insert(mpp("226.205.192.0/22"), 2)
	t.Log(rt2.dumpString())

	val, ok := rt2.Get(mpp("10.0.0.0/8"))
	if val != 1 {
		t.Errorf("Get: expected (%v, %v), got (%v,%v)", 1, true, val, ok)
	}

	val, ok = rt2.Get(mpp("226.205.192.0/22"))
	if val != 2 {
		t.Errorf("Get: expected (%v, %v), got (%v,%v)", 1, true, val, ok)
	}
}
...
func TestDelete2(t *testing.T) {
	...
	t.Run("intermediate_no_routes", func(t *testing.T) {
		...
		checkSize2(t, tbl, 7) // 2 roots, 3 intermediate, 2 leaves
		...
		checkSize2(t, tbl, 5) // 2 roots, 2 intermediates, 1 leaf
	})
	...
	t.Run("intermediate_with_route", func(t *testing.T) {
		...
		checkSize2(t, rtbl, 7) // 2 roots, 2 intermediates, 2 leaves
		...
		checkSize2(t, rtbl, 5) // 2 roots, 1 full, 1 intermediate, 1 leaf
	})
	...
	t.Run("intermediate_many_leaves", func(t *testing.T) {
		...
		checkSize2(t, rtbl, 9) // 2 roots, 4 intermediate, 3 leaves
		rtbl.Delete(mpp("192.180.0.1/32"))
		checkRoutes2(t, rtbl, []tableTest{
			{"192.168.0.1", 1},
			{"192.180.0.1", -1},
		...
		checkSize2(t, rtbl, 7) // 2 roots, 3 intermediate, 2 leaves
	})
	...
	t.Run("nosuchprefix_missing_child", func(t *testing.T) {
		...
		checkSize2(t, rtbl, 5)           // 2 roots, 2 intermediate, 1 leaf
	...
	t.Run("intermediate_with_deleted_route", func(t *testing.T) {
		...
		checkSize2(t, rtbl, 5) // 2 roots, 1 intermediate, 1 full, 1 leaf
...
func TestSupernets2(t *testing.T) {
	t.Parallel()
