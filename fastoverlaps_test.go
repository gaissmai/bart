...
func TestFastRegressionOverlaps(t *testing.T) {
	t.Parallel()

	t.Run("overlaps_divergent_children_with_parent_route_entry", func(t *testing.T) {
		t.Parallel()
		t1, t2 := new(Fast[int]), new(Fast[int])
...
func TestFastOverlapsCompare(t *testing.T) {
	t.Parallel()
	n := workLoadN()
...
func TestFastOverlapsPrefixCompare(t *testing.T) {
	t.Parallel()
	n := workLoadN()
...
func TestFastOverlapsChildren(t *testing.T) {
	t.Parallel()
	pfxs1 := []netip.Prefix{
		// pfxs
		mpp("10.0.0.0/8"),
...
func BenchmarkFastFullTableOverlaps4(b *testing.B) {
	f1 := new(Fast[any])

	for _, route := range routes4 {
		f1.Insert(route.CIDR, nil)
	}
...
func BenchmarkFastFullTableOverlaps6(b *testing.B) {
	f1 := new(Fast[any])
