...
func (t *liteTable[V]) flatSorted() golden.TableSlice[V] {
...
func TestTableNilArguments_liteTable(t *testing.T) {
	t.Parallel()

	t.Run("mustPanicNilArgs", func(t *testing.T) {
		t.Parallel()
		tbl := new(liteTable[bool])
...
func TestZonedLookup_liteTable(t *testing.T) {
	t.Parallel()

	check := func(t *testing.T, table *liteTable[string], ip netip.Addr, want string, wantOK bool) {
		t.Helper()
...
func TestTableExtra_liteTable(t *testing.T) {
	t.Parallel()

	// 1. Equal: checking self equality
	t1 := new(liteTable[int])
