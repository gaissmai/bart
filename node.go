...
type node[V any] struct {
	...
	//   - *leafNode[V]   -> path-compressed terminal node (depth < maxDepth - 1)
	//   - *fringeNode[V] -> path-compressed terminal node (depth == maxDepth - 1, stride-aligned: /8, /16, /24 ... /128))
	//
...
// reset removes all prefixes and children from the node,
// releasing any held references and preparing it for reuse.
//
// The node itself is retained and not deallocated. The method resets
// the two Sparse Arrays (prefixes and children) in place by clearing
// their BitSets and truncating their Items slices.
//
// After clear(), the node behaves as if freshly allocated via new(node[V]).
func (n *node[V]) reset() {
	// clear prefixes
