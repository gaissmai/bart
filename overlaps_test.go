...
// FuzzOverlaps validates Overlaps(o) vs ground-truth for all table types.
func FuzzOverlaps(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed uint64, n1, n2 int) {
		...
		aItems := randomPrefixes(prng, n1) // invariant: no duplicates
		bItems := randomPrefixes(prng, n2)

		aPfx := make([]netip.Prefix, len(aItems))
		for i, it := range aItems {
			aPfx[i] = it.pfx
		}
		bPfx := make([]netip.Prefix, len(bItems))
		for i, it := range bItems {
			bPfx[i] = it.pfx
		}

		gt := expectedOverlaps(aPfx, bPfx)
		...
		// Table
		{
			...
			for _, it := range aItems {
				t1.Insert(it.pfx, it.val)
			}
			for _, it := range bItems {
				t2.Insert(it.pfx, it.val)
			}
			if got := t1.Overlaps(t2); got != gt {
				t.Fatalf("Table.Overlaps mismatch: want %v, got %v", gt, got)
			}
		}
		...
		// Fast
		{
			...
			f2 := new(Fast[int])
			for _, it := range aItems {
				f1.Insert(it.pfx, it.val)
			}
			for _, it := range bItems {
				f2.Insert(it.pfx, it.val)
			}
			if got := f1.Overlaps(f2); got != gt {
				t.Fatalf("Fast.Overlaps mismatch: want %v, got %v", gt, got)
			}
		}

		// liteTable (payload-less semantics—overlap depends only on prefixes)
		{
			lt1 := new(liteTable[int])
			lt2 := new(liteTable[int])
			for _, it := range aItems {
				lt1.Insert(it.pfx, 0)
			}
			for _, it := range bItems {
			...
			if got := lt1.Overlaps(lt2); got != gt {
				t.Fatalf("liteTable.Overlaps mismatch: want %v, got %v", gt, got)
			}
		}
		...
		// Lite (prefix-only)
		{
			...
			if got := L1.Overlaps(L2); got != gt {
				t.Fatalf("Lite.Overlaps mismatch: want %v, got %v", gt, got)
			}
		}
	})
}

// FuzzOverlapsPrefix validates OverlapsPrefix(q) vs ground-truth for all table types.
// For each query, it checks that OverlapsPrefix matches any(a.Overlaps(q)).
func FuzzOverlapsPrefix(f *testing.F) {
	// Seed corpus
	...
	f.Fuzz(func(t *testing.T, seed uint64, n, nq int) {
		...
		items := randomPrefixes(prng, n)  // table contents
		query := randomPrefixes(prng, nq) // queries, use .pfx

		contents := make([]netip.Prefix, len(items))
		...
		// Table
		{
			...
			for _, it := range items {
				tbl.Insert(it.pfx, it.val)
			}
			for _, q := range query {
				gt := expectedOverlapsPrefix(contents, q.pfx)
				got := tbl.OverlapsPrefix(q.pfx)
				if got != gt {
					t.Fatalf("Table.OverlapsPrefix(%v) mismatch: want %v, got %v", q.pfx, gt, got)
				}
			}
		}
		...
		// Fast
		{
			fst := new(Fast[int])
			for _, it := range items {
				fst.Insert(it.pfx, it.val)
			}
			for _, q := range query {
				gt := expectedOverlapsPrefix(contents, q.pfx)
				got := fst.OverlapsPrefix(q.pfx)
				if got != gt {
					t.Fatalf("Fast.OverlapsPrefix(%v) mismatch: want %v, got %v", q.pfx, gt, got)
				}
			}
		}

		// liteTable (presence only)
		{
			...
			for _, q := range query {
				gt := expectedOverlapsPrefix(contents, q.pfx)
				got := lt.OverlapsPrefix(q.pfx)
				if got != gt {
					t.Fatalf("liteTable.OverlapsPrefix(%v) mismatch: want %v, got %v", q.pfx, gt, got)
				}
			}
		}
		...
		// Lite (prefix-only)
		{
			...
			for _, q := range query {
				gt := expectedOverlapsPrefix(contents, q.pfx)
				got := L.OverlapsPrefix(q.pfx)
				if got != gt {
					t.Fatalf("Lite.OverlapsPrefix(%v) mismatch: want %v, got %v", q.pfx, gt, got)
				}
			}
		}
	})
