...
func TestAsSliceFunc(t *testing.T) {
	t.Parallel()
	all := func(uint) bool { return true }
	none := func(uint) bool { return false }
	even := func(u uint) bool { return u%2 == 0 }
	odd := func(u uint) bool { return u%2 == 1 }
	...
	testCases := []struct {
		...
		},
		{
			...
			buf:      make([]uint, 0, 512),
			filter:   even,
			wantData: []uint{130, 190, 250, 300, 380, 420, 480},
		},
		{
			name:     "odd",
			set:      []uint{1, 65, 130, 190, 250, 300, 380, 420, 480, 511},
	...
	for _, tc := range testCases {
		...
		buf := b.AsSliceFunc(tc.buf, tc.filter)

		if !slices.Equal(buf, tc.wantData) {
			t.Errorf("AsSliceFunc, %s: returned buf is not equal as expected:\ngot:  %v\nwant: %v",
				tc.name, buf, tc.wantData)
		}
	}
}
