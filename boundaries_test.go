...
func TestBoundaryBehavior_DefaultRoutes(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("Table_"+tc.name, func(t *testing.T) {
			...
			if tc.prefix.Addr().Is4() {
				nextHop = mpa("192.168.1.1")
			} else {
				nextHop = mpa("fe80::1")
			}
			...
			// Test Modify
			tbl.Modify(tc.prefix, func(old *routeEntry, found bool) (*routeEntry, bool) {
				if !found {
...
func TestBoundaryBehavior_HostRoutes(t *testing.T) {
	...
	testCases := []struct {
		...
		{"IPv4_Host", mpp("203.0.113.5/32"), mpa("10.0.0.1")},
		{"IPv6_Host", mpp("2001:db8::42/128"), mpa("2001:db8::1")},
	}
...
func TestBoundaryBehavior_TransitionPaths(t *testing.T) {
	...
	t.Run("Table_"+"IPv4_Transitions", func(t *testing.T) {
		t.Parallel()
		...
		for _, r := range routes {
			...
			route := newRoute(mpa("10.0.0.1"), "eth0", r.metric)
	...
	t.Run("Table_"+"IPv6_Transitions", func(t *testing.T) {
		t.Parallel()
		tbl := new(Table[*routeEntry])
