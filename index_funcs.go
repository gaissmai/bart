...
func baseIndexToPrefixPrecalc() {
	...
	// idx == 0 is invalid for baseIndex, start with 1
	for idx = 1; idx < maxNodePrefixes; idx++ {
		lz := bits.LeadingZeros(idx)
		bits := strconv.IntSize - lz - 1
		addr := idx & (0xFF >> (stride - bits)) << (stride - bits)

		baseIdx2Pfx[idx].addr = addr
		baseIdx2Pfx[idx].bits = bits
	}
}
